...
exports.getAUsers = catchAsync(async (req, res, next) => {
  const userId = req.params.id; // Corrected from req.param.id
...
exports.getAllUsersReview = catchAsync(async (req, res, next) => {
  ...
  res.status(200).json({
    status: "success",
    data: review, // Return the array directly
  });
});
...
exports.usersreview = catchAsync(async (req, res, next) => {
  ...
  const review = await Review.create({
    ...
    photo: user.photo,
    rating: Number(req.body.rating), // Include the rating field here
  });
...
exports.updateReview = catchAsync(async (req, res, next) => {
  const review = await Review.findOneAndUpdate(
    ...
    { 
      review: req.body.review,
      rating: req.body.rating, // Update the rating as well
    },
